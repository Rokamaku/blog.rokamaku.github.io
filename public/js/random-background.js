...
function createLoadingOverlay() {
  ...
  if (!document.getElementById('background-loading-overlay')) {
    ...
    if (document.body) {
      ...
      if (DEBUG)
        console.log('Body not available, adding overlay to document')
      const bodyObserver = new MutationObserver((mutations) => {
        if (document.body) {
          if (overlay.parentNode === document.documentElement) {
            document.body.appendChild(overlay)
            if (DEBUG)
              console.log('Moved overlay to body')
          }
...
async function fetchBackgroundImages() {
  ...
  try {
    if (DEBUG)
      console.log('Fetching light mode background images...')
    ...
    if (lightResponse.ok) {
      ...
      if (DEBUG)
        console.log('Using fallback light mode images')
    ...
    if (DEBUG)
      console.log('Fetching dark mode background images...')
    ...
    if (darkResponse.ok) {
      ...
      if (DEBUG)
        console.log('Using fallback dark mode images')
    ...
    if (DEBUG)
      console.log('Loading stored background or setting random one')
...
window.onload = function () {
  if (DEBUG)
    console.log('Window loaded, initializing background loading')
...
document.addEventListener('astro:before-swap', () => {
  if (DEBUG)
    console.log('Astro page transition starting')
...
function applyExistingBackground() {
  ...
  if (storedBackground) {
    ...
    if (DEBUG)
      console.log('Applying existing background:', storedBackground)
...
document.addEventListener('theme-changed', () => {
  if (DEBUG)
    console.log('Theme changed, loading appropriate background')
