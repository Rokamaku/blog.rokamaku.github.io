...
const r2BaseUrl = window.PUBLIC_CLOUDFLARE_DOMAIN ? `https://${window.PUBLIC_CLOUDFLARE_DOMAIN}` : ''
...
async function setBackgroundWithAnimation(imageUrl, isDarkMode, showLoadingOverlay = true) {
...
async function fetchBackgroundImages() {
  ...
  if (isBackgroundLoading) {
    if (DEBUG) console.log('Background loading already in progress, skipping duplicate initialization')
    return
  }
  ...
  if (DEBUG) console.log('Loading overlay created, starting background loading sequence')
  ...
  try {
    ...
    if (lightResponse.ok) {
      ...
      if (DEBUG) console.log(`Loaded ${lightBackgroundImages.length} light mode images`)
    ...
    if (darkResponse.ok) {
      ...
      if (DEBUG) console.log(`Loaded ${darkBackgroundImages.length} dark mode images`)
...
async function loadStoredBackground(isInitialLoad = false) {
  ...
  if (storedBackground) {
    ...
    await setBackgroundWithAnimation(storedBackground, isDarkMode, useLoadingOverlay)
  } else {
...
async function setRandomBackground(showLoadingOverlay = true) {
  ...
  await setBackgroundWithAnimation(selectedImage, isDarkMode, showLoadingOverlay)
